...
const buildJSONresult = function(err, warn, inf, metadata) {
    err.every(insertDetailMessage);
    warn.every(insertDetailMessage);
    inf.every(insertDetailMessage);
...
function getDetailMessage(data) {
    return data && data.name && l10n.assembleData(null, data.name, data.key, data.extra).message;
}

function insertDetailMessage(obj) {
    var detailMessage = getDetailMessage(obj);
    if (detailMessage) {
