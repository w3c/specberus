...
const files2 = fs.readdirSync(`${__dirname}/profiles/SUBM/`);
const profiles = {};
...
[...files1.map(x => `TR/${x}`), ...files2.map(x => `SUBM/${x}`)].forEach(i => {
    const valid = /((TR|SUBM)\/([A-Z][A-Z-]*[A-Z](-Echidna)?))\.js$/.exec(i);
...
exports.REC_TEXT = {
    ...
    SOTD_C_COR_ADD:
        'It includes candidate change(s)?, introducing substantive change(s)? and new feature(s)? since the previous Recommendation.',
};
