    ...
    for (var t in rules)
        if ('*' !== t && !profileFound)
            for (var p in rules[t].profiles) {
                if (rules[t].profiles[p].name === "Candidate Recommendation Snapshot" || rules[t].profiles[p].name === "Candidate Recommendation Draft")
                    rules[t].profiles[p].name = "Candidate Recommendation";
