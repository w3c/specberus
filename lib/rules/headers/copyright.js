...
function getLatestCopyrightMatchRegex(sr, licenseTexts, shortname) {
    ...
    const regularStart =
        '^Copyright [©|&copy;] (?:(?:199\\d|20\\d\\d)-)?&YEAR *World Wide Web Consortium';
    const exceptionStart = copyrightExceptions[0]; // ^Copyright [©|&copy;] (?:(?:199\\d|20\\d\\d)-)?&YEAR *International Digital Publishing Forum and World Wide Web Consortium
    const startRex = new RegExp(
...
function checkLatestCopyright(sr, copyright, licenseTexts, shortname) {
    const matchRegex = getLatestCopyrightMatchRegex(
        sr,
        licenseTexts,
        shortname
    );
...
export async function check(sr, done) {
    ...
    const shortname = sr.getShortname();
    let exceptionRule;
    copyrightExceptions.forEach(copyrightException => {
        if (copyrightException.shortnames.indexOf(shortname) > -1)
            exceptionRule = copyrightException;
    });
    console.log('\nexceptionRule: ', exceptionRule);

    checkLatestCopyright(sr, copyright, licenseTexts, exceptionRule);
