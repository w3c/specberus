...
// Some document like epub-33 uses special copyright, as configured in copyright-exception.json
function checkSpecialCopyright(sr, copyright, specialCopyright, shortname) {
    console.log('\n\nspecial copyright for ', shortname);
    const year = (sr.getDocumentDate() || new Date()).getFullYear();

    const domHtml = sr.norm(copyright.innerHTML.replace(/'/g, `"`));
    console.log('\ndomHtml: ', domHtml);
    const specHtml = sr.norm(
        specialCopyright.copyright.replace(/&YEAR/g, year).replace(/'/g, `"`)
    );
    console.log('\nspecHtml: ', specHtml);
    if (domHtml !== specHtml) {
        sr.error(self, 'exception-no-html', { html: specHtml, shortname });
    }
}
