    ...
    if (copyright) {
        ...
        chartersData.forEach(charterData => {
            if (!allowedLicenses.length) {
                // push to the array if allowedLicenses is empty. Use the first licenseObj as base.
                charterData['doc-licenses'].forEach(licenseObj => {
                    allowedLicenses.push(licenseObj.uri);
                });
            } else {
                // Find union of multiple groups' license. Delete license in base if not adopted by other groups.
                allowedLicenses = allowedLicenses.filter(allowedLicense =>
                    // this allowedLicense is also adopted by group
                    charterData['doc-licenses'].some(
                        licenseObj => allowedLicense === licenseObj.uri
                    )
                );
            }
        });
        ...
        let licenseInDoc;
        if (!regResult) sr.error(self, 'no-match', { rex });
        else [, licenseInDoc] = regResult;
