...
exports.check = async function (sr, done) {
    ...
    if (copyright) {
        ...
        const licenses = [...charterData.map((c => c['doc-licenses'].map(l => l.uri)))]; // creates an array of license URIs per group -> array of arrays
        const allowedLicenses = licenses.reduce((a, b) => a.filter(c => b.includes(c))); // find the intersection of all arrays
