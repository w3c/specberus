...
exports.check = function (sr, done) {
    var copyright = sr.jsDocument.querySelector('body div.head p.copyright');
    if (copyright) {
        var year = (sr.getDocumentDate() || new Date()).getFullYear();
        var text = sr.norm(copyright.textContent);
        var startRex =
            '^Copyright (©|&copy;) (?:(?:199\\d|20\\d\\d)-)?' +
            year +
            ' *W3C(®|&reg;) \\(MIT, ERCIM, Keio, Beihang\\)';
        var isDualLicensed = /CC-BY/.test(text);
        var w3cOnlyStatement;
        var dualLicenseStatement =
            ', Some Rights Reserved: this document is dual-licensed, CC-BY and W3C Document License';
        var endRex =
            '\\. W3C liability, trademark,? and (document use|permissive document license) rules apply\\.$';
        var isPermissiveLicense = /permissive document license/.test(text);
        ...
        var rex = new RegExp(
            startRex +
                (isDualLicensed ? dualLicenseStatement : w3cOnlyStatement) +
                endRex
        );
        ...
        for (var href in links) {
            var sel = "a[href='" + href + "']";
            var lnk = copyright.querySelector(sel);
            if (!lnk || sr.norm(lnk.textContent) !== links[href]) {
