...
exports.check = function (sr, cb) {
    ...
    if (prevRequired && vPrev === -1) err(name, "Previous Version is missing.");
    if (vThis > vLate) err(name, "This Version must be before Latest Version.");
    if (vPrev > -1 && vLate > vPrev) err(name, "Latest Version must be before Previous Version.");
    ...
    if (vThis > -1) {
        ...
        var matches = linkThis.attr("href").match(/^https?:\/\/www\.w3\.org\/TR\/(\d{4})\/[A-Z]+-(.+)-(\d{4})(\d\d)(\d\d)\/?$/)
    ...
    if (vPrev > -1) {
        ...
        if (linkPrev.attr("href") !== linkPrev.text())
            err(name, "Link href and text differ for Previous Version.");

        var matches = linkPrev.attr("href").match(/^https?:\/\/www\.w3\.org\/TR\/\d{4}\/[A-Z]+-(.+)-\d{8}\/?$/);
        if (matches) {
            var sn = matches[1];
            if (sn !== shortname) err(name, "Short names differ between This and Previous Versions.");
        }
        else err(name, "Wrong syntax for Previous Version link.");
    }
