    ...
    if (dts.This) {
        ...
        if (exist) {
            ...
            if (matches) {
                ...
                // Require new /TR shortnames to use lowercase. No RY published yet, so this rule applies to all RYs.
                const needLowercase =
                    sr.config.track === 'Registry' || sr.config.longStatus === 'First Public Working Draft' || (sr.config.longStatus === 'Group Draft Note' && await sr.isFP() || (sr.config.status === 'SUBM' && await sr.isFP()));
