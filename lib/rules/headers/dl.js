...
exports.check = async function (sr, done) {
        ...
        if (exist) {
            ...
            if (matches) {
                ...
                if (historyShortname !== shortname) {
                    ...
                    try {
                        const res = await superagent.get(historyHref);
                        historyStatusCode = res.statusCode;
                    } catch (err) {
                        historyStatusCode = err.status;
                    }
                    if (
                        sr.config.previousVersion &&
                        ...
                        const previousShortname =
                            linkHistory &&
                            linkHistory.dataset('previous-shortname');
                        if (previousShortname) {
                            sr.warning(
                                historyError,
                                'this-previous-shortname',
                                {
                                    previousShortname,
                                    thisShortname: shortname,
                                }
    ...
    //                         previousURI
    //                 ) {
    // TODO is 'same-day-publication' warning needed?
