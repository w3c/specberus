...
exports.check = function (sr, done) {
    ...
    if (dts.Latest) {
        ...
        if (matches) {
            var sn = matches[1];
            sr.data.latestVersion = $linkLate.text();
    ...
    if (dts.Previous) {
        ...
        if (matches) {
            var sn = matches[1];
            sr.data.previousVersion = $linkPrev.text();
    ...
    if (seenEditors) {
        sr.data.editorIDs = sr.data.editorIDs || [];
        sr.$('dd[data-editor-id]').each(function() {
            sr.data.editorIDs.push(sr.$(this).attr('data-editor-id'));
        });
    }
    else {
        err("editor");
