...
exports.check = function (sr, done) {
    ...
    var rescinds = sr.config.rescinds;
    var obsoletes = sr.config.obsoletes;
    var supersedes = sr.config.supersedes;
    ...
    if (dts.This) {
        ...
        if (exist) {
            ...
            if (matches) {
                ...
                var year2 = matches[3] * 1;
                var month = matches[4] * 1;
                var day = matches[5] * 1;
                shortname = matches[2];
                const pattern = /-?\d[\d.]*$/;
    ...
    if (dts.Previous) {
        ...
        if (exist) {
            ...
            matches = (linkPrev.getAttribute('href') || '')
                ...
                .match(prevRex);
            if (matches) {
                if (matches[3] >= 20160801 && matches[1] !== 's')
    ...
    if (dts.Editor && dts.Editor.dd && dts.Editor.dd.length) {
        ...
        var missingElement = editors.filter(
        ...
        PowerPromise.all([fetch(previousURI), fetch(latestURI)]).then(
            function (bodies) {
                if (!bodies || bodies.length !== 2) {
                    ...
                    var latestDts = sr.extractHeaders(latestDl);
                    if (
                        latestDts.This &&
