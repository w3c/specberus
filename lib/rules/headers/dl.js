...
exports.check = function (sr, done) {
    ...
    var linkImple = dts.Implementation && dts.Implementation.dd.querySelector("a");
    ...
    if (impleExist && !linkImple.href.trim().toLowerCase().startsWith('https://')) {
        sr.error(self, "implelink-should-be-https", {link: linkImple});
    }

    // check "Editor's draft" link
    if (dts.EditorDraft) {
        var editorsDraftElement = dts.EditorDraft.dd.querySelector("a");
        const exist = checkLink({
            sr,
            rule: self,
