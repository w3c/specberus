...
exports.check = function (sr, done) {
    ...
    var dts = sr.extractHeaders(sr.jsDocument.querySelector("body div.head dl"));
    ...
    if (dts.This) {
        ...
        if (!linkThis) {
            sr.error(self, "this-link-not-found");
        } else {
            const text = linkThis.textContent.trim();
            const href = linkThis.getAttribute("href").trim();
            if (href !== text)
                sr.error(self, "this-link", {text, href});
        }
        ...
        var vThisRex = "^https:\\/\\/www\\.w3\\.org\\/" +
                       ...
                       "-(.+)-(\\d{4})(\\d\\d)(\\d\\d)\\/?$";
        matches = (linkThis.getAttribute("href") || "").trim().match(new RegExp(vThisRex));
        ...
        if (matches) {
            ...
            const pattern = /-?\d[\d.]*$/
            ,     dashPattern = /\d[\d.]*-/;
            seriesShortname = shortname.replace(pattern, '').replace(dashPattern, '-');
            if (docDate) {
    ...
    if (dts.Latest) {
        ...
        if (!linkLate) {
            sr.error(self, "latest-link-not-found");
        } else {
            const text = linkLate.textContent.trim();
            const href = linkLate.getAttribute("href").trim();
            if (href !== text)
                sr.error(self, "latest-link", {text, href});
        }
    ...
    if (dts.Previous) {
        ...
        if (!linkPrev) {
            sr.error(self, "previous-link-not-found");
        } else {
            const text = linkPrev.textContent.trim();
            const href = linkPrev.getAttribute("href").trim();
            if (href !== text)
                sr.error(self, "previous-link", {text, href});
        }
    ...
    if (dts.Rescinds) {
        ...
        if (!linkResc) {
            sr.error(self, "rescinds-link-not-found");
        } else {
            const text = linkResc.textContent.trim();
            const href = linkResc.getAttribute("href").trim();
            if (href !== text)
                sr.error(self, "rescinds-link", {text, href});
        }

        matches = (linkResc.getAttribute("href") || "").trim().match(/^https:\/\/www\.w3\.org\/TR\/\d{4}\/REC-(.+)-\d{8}\/?$/);
        if (matches) {
            sn = matches[1];
    ...
    if (dts.Obsoletes) {
        ...
        if (!linkObsl || linkObsl.getAttribute("href") !== linkObsl.textContent) sr.error(self, "obsoletes-link");

        matches = (linkObsl.getAttribute("href") || "").trim().match(/^https:\/\/www\.w3\.org\/TR\/\d{4}\/REC-(.+)-\d{8}\/?$/);
        if (matches) {
            sn = matches[1];
    ...
    if (dts.Supersedes) {
        ...
        if (!linkSpsd || linkSpsd.getAttribute("href") !== linkSpsd.textContent) sr.error(self, "supersedes-link");

        matches = (linkSpsd.getAttribute("href") || "").trim().match(/^https:\/\/www\.w3\.org\/TR\/\d{4}\/REC-(.+)-\d{8}\/?$/);
        if (matches) {
            sn = matches[1];
    ...
    if (dts.Implementation) {
        var linkImple = dts.Implementation.dd.querySelector("a") && dts.Implementation.dd.querySelector("a").href;
        if (needImple && !linkImple ) sr.error(self, "implelink-not-found");
        if (linkImple && !linkImple.trim().toLowerCase().startsWith('https://')) {
    ...
    if (dts.EditorDraft) {
        var editorsDraft = dts.EditorDraft.dd.querySelector("a") && dts.EditorDraft.dd.querySelector("a").href;
        if (editorsDraft && !editorsDraft.trim().toLowerCase().startsWith('https://')) {
            sr.error(self, "editors-draft-should-be-https", {link: editorsDraft});
        }
    }
    ...
    if (dts.Editor && dts.Editor.dd && dts.Editor.dd.length) {
        ...
        var missingElement = editors.filter(editor => !editor.dataset.editorId);
        if (missingElement.length) {
            sr.error(editorError, 'editor-missing-id', {
                names: missingElement.map(editor => editor.textContent).join(', ')
            });
        }
    } else {
        ...
        PowerPromise.all([fetch(previousURI), fetch(latestURI)]).then(function (bodies) {
            if (!bodies || 2 !== bodies.length) {
                ...
                if (latestDts.This &&
                    ...
                    latestDts.Previous.dd.querySelector("a").href === previousURI) {
