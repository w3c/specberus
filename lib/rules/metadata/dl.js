...
export async function check(sr, done) {
    ...
    const $linkLate = dts.Latest ? dts.Latest.$dd.find('a') : null;
    if ($linkLate) {
    ...
    const $linkHistory = dts.History ? dts.History.$dd.find('a') : null;

    if ($linkHistory) {
        result.history = $linkHistory.attr('href').trim();
        result.previousVersion = await sr.getPreviousVersion();
        previousShortname = $linkHistory.attr('data-previous-shortname');
    }

    const $linkEd = dts.EditorDraft ? dts.EditorDraft.$dd.find('a') : null;
