...
exports.check = function (sr, done) {
    ...
    function isFirstPublic(sr) {
        dl.check(sr, function (result) {
            var previousVersion = (result && result.previousVersion) || '';
            console.log(chalk.red('result: ======='));
            console.log(result);
        });
        // var headers = sr.extractHeaders(sr.$("body div.head dl").first());

        return false;
    }

    if (id) {
        id = isFirstPublic(sr) ? 'FP' + id : id;
        var meta = {profile: id};
