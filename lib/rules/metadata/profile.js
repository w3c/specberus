...
exports.check = function(sr, done) {
    ...
    function assembleMeta(id, sr) {
        ...
        if (["CR", "CRD", "PR", "REC"].indexOf(id) > -1) {
            ...
            if (dts.Implementation) {
                meta.implementationReport = dts.Implementation.dd.querySelector("a").getAttribute('href');
            }
        }
        if (id === 'REC') {
    ...
    var checkRecType = function (sr) {
        ...
        var isCR = h2Element && h2Element.textContent.indexOf("Candidate Recommendation") > 0;

        var h3Element = sr.jsDocument.querySelector('body div.head h2 + h3');
        if (isCR) {
            return (h3Element && h3Element.textContent === "Draft") ? "CRD" : "CR";
        } else {
            return "REC";
        }
    };
