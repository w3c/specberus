...
exports.check = function (sr, done) {
    ...
    function assembleMeta(id, sr) {
        ...
        if (reviewStatus.has(id)) {
            ...
            var dates = dueDate && dueDate.valid;
            var res = dates[0];
    ...
    var checkPreviousVersion = function (sr) {
        return new PowerPromise(function (resolve) {
            var dl = sr.jsDocument.querySelector('body div.head dl');
            var dts = sr.extractHeaders(dl);
            var linkPrev = dts.Previous
                ? dts.Previous.dd.querySelector('a')
                : '';
            var shortname;
            var specExists;

            var linkLatest = dts.Latest
    ...
    var checkRecType = function (sr) {
        ...
        if (
            h2Element &&
            h2Element.textContent.indexOf('Candidate Recommendation') > 0
        ) {
            return h2Element.textContent.indexOf('Draft') > 0 ? 'CRD' : 'CR';
        }
        return 'REC';
    };
