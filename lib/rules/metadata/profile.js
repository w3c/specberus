...
exports.check = async function (sr, done) {
    ...
    if (!stateEle) {
        throw new Error(
            'Cannot find the <code>&lt;p id="w3c-state"&gt;</code> element for profile and date.<br><br>Please make sure the <code>&lt;stateEle&gt;W3C @@Profile, DD Month Year&lt;/stateEle&gt;</code> element can be selected by <code>document.getElementById(\'w3c-state\')</code>; <br>If you are using bikeshed, please update to the latest version.'
        );
    }
    ...
    async function isFP(sr) {
        const previousLink = await sr.getPreviousVersion(sr);
        if (!previousLink) {
            const dts = sr.extractHeaders();
            const shortnameChange =
                dts.History &&
                dts.History.dd.querySelector('a').dataset['previous-shortname'];
            return !shortnameChange;
        }
        return false;
    }
    ...
    if (id) {
        if (/-NOTE/.test(id)) {
            // todo rename isFP(sr) from checkPreviousVersion(sr);
            const hasPreviousVersion = await isFP(sr);
        ...
        if (candidate.indexOf("editor's draft") > -1) {
            ...
            sortedProfiles.forEach(category => {
                ...
                category.profiles.forEach(profile => {
                    // TODO: fix error msg
                    profileList += `<li>&lt;stateEle&gt; for <a href="./doc/rules/?profile=${profile.abbr}#dateTitleH2">${profile.name}</a></li>`;
                });
                profileList += '</ul>';
            });
            sr.throw(
                `Pubrules is having a hard time identifying the profile of the document "${docTitle}" from its &lt;stateEle&gt;. Please make sure the &lt;stateEle&gt; is in a valid format: <pre><code>&lt;stateEle&gt;W3C @@type of the document@@ DD Month YYYY&lt;/stateEle&gt;</code></pre>${profileList}`
            );
        } else {
            sr.throw(
