...
exports.check = async function (sr, done) {
    ...
    if (sotd) {
        ...
        await Promise.all(fetchGroupNames(sr.getDelivererIDs())).then(function (groupNames) {
            isIGDeliverable = groupNames.some(groupName => groupName.endsWith('Interest Group'));
        }).catch(function (e) {
            sr.error(self, "no-response", { error: JSON.stringify(e) });
            return done();
        });

        var pp = findPP(util.filter(sotd, "p").concat(...sotd.querySelectorAll("p")), sr, isIGDeliverable);
        if (!pp) {
            sr.error(self, "no-pp");
            return done();
        }
