...
function buildWanted(groups, sr, isIGDeliverable) {
    ...
    var wanted;
    var result = {};
    var isPP2002 = config.patentPolicy === 'pp2002';
    var isWGNote = config.longStatus === 'Working Group Note';
    var isAmended = config.amended;
    if (isPP2002)
        wanted =
        ...
        if ((config.recTrackStatus && config.noRecTrack) || isIGDeliverable)
            wanted += noRec;
...
function findPP(candidates, sr, isIGDeliverable) {
    ...
    var wanted = buildWanted(groups, sr, isIGDeliverable);
    var expected = wanted.text;
    Array.prototype.some.call(candidates, (p) => {
        var text = sr.norm(p.textContent);
...
function fetchGroupNames(deliverIds) {
    ...
    deliverIds.forEach((deliverId) => {
        promiseArray.push(
            new PowerPromise(function (resolve, reject) {
...
exports.check = async function (sr, done) {
    ...
    const patent =
        sr.config.patentPolicy === 'pp2020'
            ? pp2020
            : sr.config.patentPolicy === 'pp2004'
            ? pp2017
            : sr.config.patentPolicy;
    // Skip patent policy check for publications from TAG
    if (sr.getDelivererIDs().includes(util.TAG_ID)) return done();

    var isIGDeliverable = false;
    ...
    if (sotd) {
        ...
        await Promise.all(fetchGroupNames(sr.getDelivererIDs()))
        ...
        pp.querySelectorAll('a[href]').forEach(function (a) {
            ...
            var text = sr.norm(a.textContent);
            if (
                patent === pp2020 &&
            ...
            if (
                /^https:\/\/www\.w3\.org\/2004\/01\/pp-impl\/\d+\/status(#.*)?$/.test(
                    href
                ) &&
        ...
        if (
            (sr.config.recTrackStatus || sr.config.noteStatus) &&
            !isWGNote &&
            !foundEssentials
        )
