...
function buildWanted(groups, sr) {
    ...
    console.log('isWGNote: ', isWGNote, ', isIGNote: ', isIGNote);
    if (isIGNote || isWGNote)
        wanted.push("Publication as an " + config.longStatus +
    ...
    if (!isIGNote) {
        if (isPP2002)
            wanted.push("This document is governed by the 24 January 2002 CPP as amended by the W3C Patent " +
                "Policy Transition Procedure\\. ");
        else {
            var start = (!isAmended) ? "This document " : "The Recommendation from which this document is derived ";
            wanted.push(start + "was produced by " +
            ...
            if (config.recTrackStatus && config.noRecTrack)
                wanted[wanted.length - 1] += ((groups.length == 2) ? "The groups do not " : "The group does not ") +
                    "expect this document to become a W3C Recommendation\\. ";
            if (config.informativeOnly && !config.noteStatus)
                wanted[wanted.length - 1] += "This document is informative only\\. ";
        }
        
        if (!isWGNote) {
            wanted[wanted.length - 1] += "W3C maintains ";
            if (groups.length < 2) {
                wanted[wanted.length - 1] += "a public list of any patent disclosures";
            } else {
                wanted[wanted.length - 1] += groups.map(function (wg) {
                    return "a public list of any patent disclosures \\(" + wg.replace(' Working Group', '') + "( Working Group)?\\)";
                }).join(" and ");
            }
            wanted[wanted.length - 1] += " made in connection with the deliverables of " +
            ...
            if (config.recTrackStatus || config.noteStatus || isPP2002)
                wanted[wanted.length - 1] += " An individual who has actual knowledge of a patent which the individual " +
                    "believes contains Essential Claim\\(s\\) must disclose the information in " +
                    "accordance with section 6 of the W3C Patent Policy\\.";
            if (isAmended)
                wanted[wanted.length - 1] += " This Amended Recommendation was produced by incorporating errata after that group closed\\.";
        }
    }

    result = wanted.map(function (item) {
        const itemReg = '^' + item + '$';
        const regex = new RegExp(itemReg);
        return {
            regex,
            text: itemReg.replace(/\\/g, '')
        }
    });
...
function findPP ($candidates, sr) {
    var $pp = null
        ...
        , expected = ''
        , passAll = true
        , wanteds = buildWanted(groups, sr)
        ;

    // TODO: WG Note as IG Note?
    // input[https://www.w3.org/TR/2020/NOTE-lpf-20200319/] 
    wanteds.forEach(function (wanted, index) {
        expected = expected + ( index + 1 ) + ': ' + wanted.text + '\n';
...
exports.check = function (sr, done) {
    ...
    if ($sotd && $sotd.length) {
        ...
        if (sr.config.longStatus !== "Interest Group Note") {
          ...
          var pp = "https://www.w3.org/Consortium/Patent-Policy/"
          ...
          ,   doAfter:      () => {}
            ...
            $pp.find("a[href]").each(function () {
                ...
                ,   text = sr.norm($a.text())
                ;
                if (href === pp && text === "W3C Patent Policy") {
                    found2017Aug1 = true;
                    return;
                }
