...
exports.check = function (sr, done) {
    ...
    if (sotd) {
        ...
        var url = deliverId.length ? 'https://api.w3.org/groups/' + deliverId[0] + '/charters' : "";
        if (url) {
            var req = sua.get(url)
            ...
            req.end(function (err, res) {
                if (err || !res.ok) {
                    ...
                    // if using PP 2020, also allow short link as "https://www.w3.org/Consortium/Patent-Policy/"
                    if (expectedPP === "https://www.w3.org/Consortium/Patent-Policy/20200915/") {
                        allowShortlinkPP = true;
                    }
                    if (allowShortlinkPP) {
                        console.log('allowShortlinkPP');
                    }
                }
            });
        }
