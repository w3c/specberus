...
function checkPPMatch(pp, expectedPP, sr, done) {

    var pp2020 = "https://www.w3.org/Consortium/Patent-Policy/20200915/"
        , pp2017 = "https://www.w3.org/Consortium/Patent-Policy/2017"
        , ppShort = "https://www.w3.org/Consortium/Patent-Policy";
    ...
    pp.querySelectorAll("a[href]").forEach(function (a) {
        var href = a.getAttribute("href")
            ...
            ;
        if (text === "W3C Patent Policy" && href === pp2020 || href === ppShort) {
            found2020 = true;
            return;
        }
        if (href === pp2017 && text === "W3C Patent Policy") {
            found2017Aug1 = true;
            return;
        }
        ...
        if (((href === pp2017 + "#def-essential") ||
            (href === pp2020 + "#def-essential") ||
            (href === ppShort + "#def-essential")) &&
            text === "Essential Claim(s)") {
            foundEssentials = true;
            return;
        }
        if (((href === pp2017 + "#sec-Disclosure") ||
            (href === pp2020 + "#sec-Disclosure") ||
            (href === ppShort + "#sec-Disclosure")) && 
    ...
    if (!isPP2002 && !found2017Aug1 && !found2020) {
        sr.error(self, "no-valid-pp");
    }
...
exports.check = function (sr, done) {
    ...
    if (sotd) {
        ...
        if (!deliverId.length) {
            sr.error(self, "no-deliver");
            return done();
        }

        var url = 'https://api.w3.org/groups/' + deliverId[0] + '/charters';
        var req = sua.get(url)
                        .set("User-Agent", "W3C-Pubrules/" + sr.version);
        req.query({ apikey: process.env.W3C_API_KEY, embed: true});
                ...
                if (charters && charters.length) {
                    charters.forEach(element => {
                        ...
                        if (docDate < endDate && docDate > startDate) {
                            expectedPP = element["patent-policy"];
                        }
                    });
                }
                checkPPMatch(pp, expectedPP, sr, done);
            }
        });
    }
};
