...
exports.check = function (sr, done) {
    ...
    ,     patent = (sr.config.patentPolicy === "pp2020") ? pp2020 : (sr.config.patentPolicy === "pp2004") ? pp2017: undefined
    ;
    ...
    if (sotd) {
        ...
        if (!deliverers.length) {
            sr.error(self, "no-deliverer");
            return done();
        }
        // assume all deliverers are operating under the same PP
        w3capi.apiKey = process.env.W3C_API_KEY;
        w3capi.group(deliverers[0]).charters().fetch({ embed: true }, (err, charters) => {
            if (err) {
               sr.error(self, "charters", { "err" : err });
               return done();
            } else {
                let charterFound = false;
                charters.forEach (c => {
                    if (docDate >= new Date(c.start) && docDate <= new Date(c.end)) {
                        charterFound = true;
                        const wgPP   = (c["patent-policy"] !== patent) ? patent : c["patent-policy"]
                        ,     possiblePP = (wgPP === pp2020) ? [ppLink, pp2020] : (wgPP === pp2017) ? [pp2017] : []
                        ;
                        ...
                        pp.querySelectorAll("a[href]").forEach(function (a) {
                            ...
                            if (wgPP === pp2020 && possiblePP.includes(href) && /(15 September 2020 )?W3C Patent Policy/.test(text)) {
                                found2020 = true;
                                return;
                            }

                            if (wgPP === pp2017 && possiblePP.includes(href) && /(1 August 2017 )?W3C Patent Policy/.test(text)) {
                                found2017 = true;
                                return;
                            }
                        ...
                        if (!isPP2002) {
                            if (wgPP === pp2017 && !found2017) sr.error(self, "no-pp2017");
                            else if (wgPP === pp2020 && !found2020) sr.error(self, "no-pp2020");
