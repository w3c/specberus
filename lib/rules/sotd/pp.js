...
function buildWanted (wgs, sr) {
    ...
    if (wgs.length < 2) {
        ...
        wanted += wgs.map(function(wg) {
                    return "a public list of any patent disclosures (" + wg + ")";
                  }).join(' and ');
    }
    wanted += " made in connection with the deliverables of " +
              ((wgs.length == 2) ? "each group; these pages also include " : "the group; that page also includes ") +
...
function findPP ($candidates, sr) {
    ...
    $candidates.each(function () {
        ...
        if (jointRegex.test(text)) {
          ...
          wgs.push(matches[1]);
          wgs.push(matches[2]);
          sr.warning(exports.name, "joint-publication");
        }
        if (text === wanted) {
            $pp = $p;
            return false;
