...
exports.check = function (sr, done) {
    ...
    if (sotd && sotd.length) {
        ...
        ;
        sotd.parentElement.querySelectorAll("p").forEach(function (p) {
            if (sr.norm(p.textContent) === boilerplate && p.querySelector("a").getAttribute("href") === procUri) {
                if (found) sr.error(self, "multiple-times", { process: proc });
                else {
                    found = true;
                }
            }
            else if (deprecatedBoilerplate === sr.norm(p.textContent) || deprecatedUri === p.querySelector('a').getAttribute('href')) {
            ...
            else if (sr.norm(p.textContent) === boilerplate && p.querySelector("a").getAttribute("href") !== procUri) {
