...
exports.check = function (sr, done) {
    ...
    if (sotd) {
        var recType = sr.getRecMetadata(sr.norm(sotd.textContent), {});
        ...
        if (recType.substantiveChanges) {
            ...
            if (correctionSection) {
                if (correctionSection.textContent !== CORRECTION) {
                    sr.error(self, 'wrong-text', { type: 'correction', text: CORRECTION });
                }
            } else {
                sr.error(self, "no-correction");
            }
        }
        ...
        if (recType.newFeatures) {
            ...
            if (additionSection) {
                if (additionSection.textContent !== ADDITION) {
