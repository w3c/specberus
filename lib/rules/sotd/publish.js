...
exports.check = function (sr, done) {
    ...
    if (sotd) {
        ...
        // check if 'candidate changes' or 'proposed changes' link in same paragraph is valid.
        if (recType) {
            ...
            // for candidate changes, candidate additions, candidate corrections.
            if (recType.cSubChanges && recType.cNewFeatures) {
                urlExpected = `${BASE_URL}#candidate-changes`;
                textExpected = /candidate change(s)?/;
            } else if (recType.cSubChanges) {
                urlExpected = `${BASE_URL}#candidate-corrections`;
