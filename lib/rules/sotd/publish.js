...
exports.check = async function (sr, done) {
    ...
    const text = `^This document was (?:produced|published) by the (.+? Working Group|Technical Architecture Group( \(TAG\))?|Advisory Board|.+? Interest Group)( and the (.+? Working Group|Technical Architecture Group( \(TAG\))?|Advisory Board|.+? Interest Group))? as a ${docType} using the ${sr.config.track} track.`;
