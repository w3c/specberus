...
exports.check = function (sr, done) {
    ...
    var docType = (sr.config.status === "CR" || sr.config.status === "CRS") ? "Candidate Recommendation " + crType : sr.config.longStatus;
        ...
        Array.prototype.some.call(sotd.querySelectorAll("p"), paragraph => {
            ...
            if (txt.match(PUBLISH)) {
                ...
                if (recType.substantiveChanges || recType.newFeatures) {
                    ...
                    var proposedFound = Array.prototype.some.call(paragraph.querySelectorAll('a'), function (ele) {
                        if (ele.textContent === PROPOSED_TEXT) {
                            if (ele.href !== PROPOSED_URL) {
                                sr.error(selfRecChanges, "url-not-match", {url: PROPOSED_URL, text: PROPOSED_TEXT});
                            }
                            return true;
                        }
                        return false;
                    ...
                    if (!proposedFound)
                        sr.error(selfRecChanges, "url-not-found", { url: PROPOSED_URL, text: PROPOSED_TEXT });
