...
exports.check = function (sr, done) {
    ...
    let text = `This document was (?:produced|published) by the (.+? Working Group|Technical Architecture Group)( and the (.+? Working Group|Technical Architecture Group))? as a ${docType}.`;

    if (sotd) {
        // If document doesn't contain proposed addition nor proposed correction, there shouldn't be any more sentences like 'It includes proposed corrections.'
        const proposedAdditionElement =
            sr.jsDocument.querySelector('.addition.proposed');
        const proposedCorrectionElement = sr.jsDocument.querySelector(
            '.correction.proposed'
        );
        if (!proposedAdditionElement && !proposedCorrectionElement) {
            // end of paragraph.
            text += '$';
        }
