...
exports.check = async function (sr, done) {
    ...
    const text = `^This document was (?:produced|published) by the (.+? Working Group|Technical Architecture Group|Advisory Board)( and the (.+? Working Group|Technical Architecture Group|Advisory Board))? as a ${docType} using the ${sr.config.track} track.`;

    // eslint-disable-next-line prettier/prettier
    // console.log( '\n\npublish.js:', sr.config.status, sr.config.longStatus, text);
    ...
    if (sotd) {
        ...
        const trackEle = Array.prototype.filter.call(sotdLinks, ele =>
            ele.textContent.match(sr.config.track)
        )[0];
        if (trackEle.href !== urlExpected) {
            sr.error(self, 'url-not-match', {
                url: urlExpected,
                text: sr.config.track,
            });
        }
        ...
        const groupsURLExpected = delivererGroups.map(
            delivererGroup =>
                `https://www.w3.org/groups/${delivererGroup.groupType}/${delivererGroup.groupShortname}/ipr`
        );
        Array.prototype.forEach.call(sotdLinks, sotdLink => {
            groupsURLExpected.forEach((groupURLExpected, index) => {
                // delete group URL from array which is found in SOTD
                if (sotdLink === groupURLExpected) groupURLExpected.pop(index);
            });
        });
        // groupsURLExpected now is a list of remaining not-found links.
        groupsURLExpected.forEach(groupURLExpected => {
            sr.error(self, 'no-homepage-link', { homepage: groupURLExpected });
        });
