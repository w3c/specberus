...
/**
 * Upgraded version of Array.includes(), supporting both RegExp and exact string comparison.
 */
function includedByReg(url: string, regArray = allowList) {
    return regArray.some(item =>
        item instanceof RegExp ? item.test(url) : item === url
    );
}
