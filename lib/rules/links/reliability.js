...
exports.check = function (sr, done) {
    Array.prototype.some.call(sr.jsDocument.querySelectorAll("a"), function (element) {
        let url;
        try {
            url = new URL(element.href, sr.url || 'https://w3.org');
        }
        catch (e) {
        ...
        unreliableServices.some(unreliableService => {
            if (domain.endsWith(unreliableService.domain) && path.match(unreliableService.path)) {
