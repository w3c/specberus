...
exports.check = function (sr, done) {
    ...
    links = links.sort().filter(function (item, pos) {
        return !pos || item != links[pos - 1];
    });
            ...
            if (sr.config.validation === 'recursive') {
                ...
                req.end(function (err1, res) {
                    ...
                    reqCSS.end(function (err2, res) {
                        if (err2 && err2.timeout === TIMEOUT)
                            sr.warning(self, 'css-timeout');
                        if (
                            res &&
                            res.header &&
                            res.header['x-w3c-validator-status'] === 'Valid'
                        )
                            isCSSValid = true;
