...
Specberus.prototype.getChartersAndPolicy = async function () {
    if (undefined !== this.charters) {
        return {
            charters: this.charters,
            patentPolicy: this.patentPolicy,
        };
    }
    ...
    if (deliverers.length) {
        ...
        deliverers.forEach((deliverer) => {
            // Skip finding charter for TAG group.
        ...
        // Find the living charter, and curresponding patentPolicy.
        // groups -> group is for joint publication.
        groupsCharters.forEach((groupChaters) => {
            groupChaters.forEach((groupCharter) => {
                if (
                    docDate >= new Date(groupCharter.start) &&
                    docDate <= new Date(groupCharter.end)
                ) {
                    patentPolicy.push(groupCharter['patent-policy']);
                    charters.push(groupCharter.uri);
                }
            });
        });
    }
    ...
    [this.patentPolicy] = patentPolicy;
    return {
        charters: this.charters,
        patentPolicy: this.patentPolicy,
    };
