...
Specberus.prototype.checkSelector = function (sel, name, done) {
    try {
        // p:contains[fooText] is jquery selector, not supported in vanilla js. Need to separate.
        var containsRegx = /:contains\('([\w\d_\-,. ]+)'\)/;
        var containsText = containsRegx.exec(sel) && containsRegx.exec(sel)[1];
        var selector = sel.replace(containsRegx, '');
        var selElement = containsText ? util.contains(this.jsDocument, selector, containsText) : this.jsDocument.querySelectorAll(sel);
...
Specberus.prototype.getDocumentDate = function () {
    ...
    ,   self = this
    ;
    Array.prototype.every.call(this.jsDocument.querySelectorAll("body div.head h2"), function (element) {
        var matches = self.norm((element).textContent).match(rex);
        if (matches) {
            self.docDate = self.stringToDate(matches[1] + " " + matches[2] + " " + matches[3]);
            self.docDateEl = self.jsDocument;
...
Specberus.prototype.getSotDSection = function () {
    ...
    var startH2, endH2
    ,   div = this.jsDocument.createElement('div')
    ,   self = this
    ,   nav = this.jsDocument.querySelector("nav#toc")
    ;
    Array.prototype.every.call(this.jsDocument.querySelectorAll("h2"), function (h2) {
        if (startH2) {
            endH2 = h2;
            return false;
        }
        if (/^Status [Oo]f [Tt]his [Dd]ocument$/.test(self.norm(h2.textContent))) {
            startH2 = h2;
        }
        return true;
    });
    if (!startH2)
        ...
        var started = false;
        Array.prototype.every.call(startH2.parentElement.children, function (element) {
            if (startH2 === element) {
                started = true;
                return true;
            }
            if (!started) return true;
            if ((endH2 === element) || nav === element) return false;
    ...
    if (!this.sotdSection || !this.sotdSection.list.length)
...
Specberus.prototype.extractHeaders = function (dl) {
  ...
  dl.querySelectorAll("dt").forEach(function (dt, idx) {
    var txt = self.norm(dt.textContent)
                  .replace(":", "")
                  .toLowerCase()
                  .replace("published ", "")
    ...
    if (EDITORS_DRAFT.test(txt) && dd.querySelector('a')) key = "EditorDraft";
    if (EDITORS.test(txt)) {
        key = "Editor";
...
Specberus.prototype.getEditorIDs = function () {
    ...
    var editors = Array.prototype.map.call(dataEditor, function (element) {
...
Specberus.prototype.getFeedbackDueDate = function() {
    ...
    ;
    if (sotd) {
        var txt        = this.norm(sotd.element.textContent)
...
Specberus.prototype.getDelivererIDs = function () {
    ...
    var ids = this.getDelivererIDsNote() || []
        ...
        , sotdLinks = sotd && sotd.element.querySelectorAll('a[href]');

    if (ids.length === 0 && sotdLinks.length > 0) {
        Array.prototype.every.call(sotdLinks, function (item) {
            var href = item.getAttribute('href')
            ,   text = self.norm(item.textContent)
            ,   found = {}
            ;
...
Specberus.prototype.getDelivererIDsNote = function () {
    ...
    ,   self = this
    ,   sotd = self.getSotDSection()
    ,   deliver = sotd && sotd.element.querySelectorAll('[data-deliverer]')
    ;
    if (deliver && deliver.length > 0) {
        deliver.forEach(function(element) {
            var deliverers = element.getAttribute('data-deliverer').trim().split(/\s+/)
            ;
...
Specberus.prototype.loadSource = function (src, cb) {
    ...
    this.source = src;
    try {
        // JSDOM.fromURL(src).then(dom => {
        //     var jsDocument = dom.serialize().window.document;
        //     cb(null, jsDocument);
        // });
        var jsdom = new JSDOM(src);
