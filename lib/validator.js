...
Specberus.prototype.error = function (rule, key, extra) {
    var name;
    if ('string' === typeof rule) name = rule;
    else name = rule.name;
    if (
        this.shortname !== undefined &&
        ...
        if ('string' === typeof rule)
            this.sink.emit('err', name, {
                key: key,
                extra: extra,
                detailMessage: l10n.assembleData(null, name, key, extra)
                    .message,
            });
        else
            this.sink.emit('err', rule, {
                key: key,
                extra: extra,
                detailMessage: l10n.assembleData(null, rule, key, extra)
                    .message,
            });
    }
};
