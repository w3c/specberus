...
Specberus.prototype.error = function (rule, key, extra) {
    ...
    if ((this.shortname !== undefined) &&
        (this.config.status === "WD" || this.config.status === "CR" || this.config.status === "CRD" || this.config.status === "NOTE") &&
        this.exceptions.has(this.shortname, name, key, extra))
        this.warning(rule, key, extra);
    else {
        if('string' === typeof rule)
            this.sink.emit('err', name, {key: key, extra: extra, detailMessage: l10n.assembleData(null, name, key, extra).message});
        else
            this.sink.emit('err', rule, {key: key, extra: extra, detailMessage: l10n.assembleData(null, rule, key, extra).message });
    }
};

Specberus.prototype.warning = function (rule, key, extra) {
    this.sink.emit('warning', rule, { key: key, extra: extra, detailMessage: l10n.assembleData(null, rule, key, extra).message });
};

Specberus.prototype.info = function (rule, key, extra) {
    this.sink.emit('info', rule, {key: key, extra: extra, detailMessage: l10n.assembleData(null, rule, key, extra).message });
};
...
Specberus.prototype.getDocumentDate = function () {
    ...
    ,   self = this
    ;
    Array.prototype.some.call(this.jsDocument.querySelectorAll("body div.head h2"), element => {
...
Specberus.prototype.extractHeaders = function (dl) {
    ...
    if (dl) {
        dl.querySelectorAll("dt").forEach(function (dt, idx) {
            ...
            else if (/^implementation report?$/.test(txt)) key = "Implementation";
...
Specberus.prototype.getDelivererIDs = function () {
    ...
    ,   sotd = this.getSotDSection()
    ,   sotdLinks = sotd && sotd.querySelectorAll('a[href]');

    if (ids.length === 0 && sotdLinks.length > 0) {
        sotdLinks.forEach(item => {
            var href = item.getAttribute('href')
            ,   text = this.norm(item.textContent)
            ,   found = {}
            ;
...
Specberus.prototype.getDelivererIDsNote = function () {
    ...
    ,   deliver = sotd && sotd.querySelectorAll('[data-deliverer]')
    ;
...
Specberus.prototype.getRecMetadata = function (meta) {
    ...
    const SOTD_ADDITION = "It includes proposed addition(s)?, introducing new feature(s)? since the previous Recommendation.";
    const SOTD_COR_ADD = "It includes proposed change(s)?, introducing substantive change(s)? and new feature(s)? since the previous Recommendation.";
