...
Specberus.prototype.checkSelector = function (sel, name, done) {
    try {
        if (!this.jsDocument.querySelectorAll(sel).length) this.error(name, "not-found");
    }
    catch (e) {
...
Specberus.prototype.getDelivererIDs = function () {
    ...
    if (ids.length === 0 && sotd && sotd.querySelectorAll('a[href]').length > 0) {
        Array.prototype.every.call(sotd.querySelectorAll("a[href]"), function (item) {
...
Specberus.prototype.getDelivererIDsNote = function() {
    ...
    ,   self = this
    ,   sotd = self.getSotDSection()
    ,   container = this.jsDocument.createElement('div').appendChild(sotd);

    if (sotd && container.querySelectorAll('[data-deliverer]').length > 0) {
        container.querySelectorAll('[data-deliverer]').forEach(function(element) {
            var deliverers = element.attr('data-deliverer').trim().split(/\s+/)
            ;
...
Specberus.prototype.loadSource = function (src, cb) {
    ...
    try {
        ...
        this.throw("Jsdom failed to parse source: " + e);
    }
};
