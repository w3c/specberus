...
Specberus.prototype.extractHeaders = function (dl) {
    ...
    // If 'dl' is set, it may comes from another document, e.g. previous document, extract everytime.
    ...
    if (dl) {
        dl.querySelectorAll('dt').forEach(function (dt, idx) {
            ...
            if (txt === 'this version') key = 'This';
                ...
                key = 'Latest';
            else if (/^history$/i.test(txt)) key = 'History';
...
Specberus.prototype.getPreviousVersion = async function () {
    ...
    // todo: use this version as shortname
    const linkHistory = dts.History
        ? dts.History.dd.querySelector('a').getAttribute('href')
        : '';
    const shortnameReg = /^https:\/\/www\.w3\.org\/standards\/history\/(.+)$/;
    const shortname =
        linkHistory.match(shortnameReg) && linkHistory.match(shortnameReg)[1];
    ...
    function shortnameHistoryP() {
        return new Promise(resolve => {
            w3capi
                ...
                .versions()
                .fetch({ embed: true }, (err, data) => resolve(data));
        });
    }
    ...
    versions.every(version => {
        const linkThis = dts.This
            ...
            : '';
        if (version.uri === linkThis) {
            // same day publication. use older version as previousVersion link.
            return true;
        }
        previousVersion = version.uri;
        return false;
    });
...
Specberus.prototype.getRecMetadata = function (meta) {
    ...
    // proposed corrections, proposed additions, proposed changes:
    ...
    // candidate corrections, candidate additions, candidate changes:
