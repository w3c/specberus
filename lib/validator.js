...
Specberus.prototype.validate = function (options) {
    ...
    if (!options.events) return console.error("[ERROR] The events option is required for reporting.");
    self.sink = options.events;
    if (this.sink.listeners('exception').length === 0) {
...
Specberus.prototype.getDocumentDate = function () {
    if (this.data.docDate) return this.data.docDate;
    ...
    this.$("body div.head h2").each(function () {
        ...
        if (matches) {
            self.data.docDate = self.stringToDate(matches[1] + " " + matches[2] + " " + matches[3]);
            self.docDateEl = self.$(this);
            return false; // no need to look further
        }
    });
    return this.data.docDate;
};
