...
Specberus.prototype.getRecMetadata = function (meta) {
    ...
    const SOTD_ADDITION = "It includes proposed addition(s)?, introducing new feature(s)? since the previous Recommentation.";
    const SOTD_COR_ADD = "It includes proposed change(s)?, introducing substantive change(s)? and new feature(s)? since the previous Recommentation.";
    var sotdText = this.norm(this.getSotDSection().textContent);
    if (sotdText.match(new RegExp(SOTD_CORRECTION + '|' + SOTD_COR_ADD)))
        meta.substantiveChanges = true;
    if (sotdText.match(new RegExp(SOTD_ADDITION + '|' + SOTD_COR_ADD)))
