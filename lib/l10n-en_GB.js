...
,   "headers.dl.not-found":                 "Cannot find the &lt;a&gt; tag or link for ${linkName}."
,   "headers.dl.link-diff":                 "${linkName}: Link href and text differ. Link href is `${href}` while text is `${text}`."
...
,   "headers.dl.this-supersedes-shortname": "<em>Shortnames</em> differ between This Version (${thisShortname}) and Supersedes this Recommendation (${obsoletesShortname})."
...
,   "links.linkchecker.not-same-folder": "This resource or link found in document (${url}) is not in the same folder as the document (${base}). Please install them in the same folder."
,   "links.linkchecker.response-error": "This resource or link found in document (${url}) is not responding ok (${status} ${text}). Please install it correctly."
    // links/compound
...
,   "links.reliability.unreliable-link": "Link for <em>\"${text}\"</em> is pointing to unreliable domain or legacy service <em>\"${link}\"</em>."
    // sotd/supersedable
