...
exports.assembleData = function (profileCode, rule, key, extra) {
    ...
    if ('string' === typeof rule)
        ...
        name = rule.name;
    if (!lang)
        throw new Error('l10n.assembleData() invoked before a locale is defined; call l10n.setLanguage() first');
    else if (!Object.prototype.hasOwnProperty.call(rules, profile))
        throw new Error(`l10n.assembleData(): unknown profile code “${profile}”`);
    else {
        if (!Object.prototype.hasOwnProperty.call(lang, name + '.' + key))
            throw new Error(`l10n.assembleData() could not interpret key “${name}.${key}”`);

        messageData.message = lang[name + '.' + key] || '';
        if (extra)
            messageData.message = messageData.message.replace(/\$\{([^}]+)\}/g, function interpolate(m, p1) {
...
exports.message = function (profileCode, rule, key, extra) {
    var result = {};
    var messageData = exports.assembleData(profileCode, rule, key, extra);
    if (!messageData)
        return;
    if (messageData.message && messageData.message.length) {
        result.message = `<div class="message">${messageData.message}</div>`;
    }
    var profile = messageData.profile;
