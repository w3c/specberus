...
var messages = {
    ...
    ,   "structure.section-ids.no-id":  "No ID for section: '${text}'."
        // structure/h2
    ,   "structure.h2.abstract":    "First <h2> after div.head must be 'Abstract'."
    ,   "structure.h2.sotd":        "Second <h2> after div.head must be 'Status of This Document'."
    ,   "structure.h2.toc":         "Third <h2> after div.head must be 'Table of Contents'."
        // style/sheet
...
exports.message = function (lang, rule, key, extra) {
    ...
    var l10n = messages[rule + "." + key];
    if (!l10n) return "@@@No such entry: " + rule + "." + key + "@@@";
    if (extra) {
        l10n.replace(/\$\{([^\}])\}/g, function (m, p1) {
