...
,   memSubm = require('./profiles/SUBM/MEM-SUBM')
,   teamSubm = require('./profiles/SUBM/TEAM-SUBM')
;
...
const retrieveProfile = function(query) {
    ...
    if (query && query.profile) {
        ...
        if (0 === Object.keys(result).length)
            ...
            if (memSubm.name === codename)
                result.body = '<p>We are working on documenting the rules here.</p>' +
                    '<p>In the meantime, please refer to' +
                    '<a href="https://www.w3.org/2005/07/pubrules?year=2015&uimode=filter&filter=Filter+pubrules&filterValues=form&docstatus=mem-subm&' +
                    'patpol=w3c&rectrack=yes&normative=yes&procrev=2005&prevrec=none">the' +
                    'old pubrules for <em>Member Submission</em></a>.</p>';
            if (teamSubm.name === codename)
                result.body = '<p>We are working on documenting the rules here.</p>' +
                    '<p>In the meantime, please refer to' +
                    '<a href="https://www.w3.org/2005/07/pubrules?year=2015&uimode=filter&filter=Filter+pubrules&filterValues=form&docstatus=team-subm&' +
                    'patpol=w3c&rectrack=yes&normative=yes&procrev=2005&prevrec=none">the' +
                    'old pubrules for <em>Team Submission</em></a>.</p>';
        }
    } else
