...
const listProfiles = function() {
    ...
    for (var p in rules)
        if ('*' !== p)
            result.push({order: rules[p].order, abbr: p, name: rules[p].name});
...
const retrieveProfile = function(query) {
    ...
    if (query && query.profile) {
        ...
        if (rules && rules.hasOwnProperty(codename)) {
            const profile = rules[codename];
...
const setUp = function(app) {
    ...
    app.get('/doc', function(req, res) {
        res.render('doc', {DEBUG: DEBUG, BASE_URI: BASE_URI, version: version, nav: nav, title: 'documentation', profiles: sortedProfiles});
    });
